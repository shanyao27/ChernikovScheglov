...
from main_page.models import User, Admin, Inspector, Medic, Department
...
class ViolationType(models.Model):
    ...
    default_sanction = models.CharField(
        max_length=30,
        choices=SANCTION_CHOICES,
        default='warning',
        verbose_name='Базовая санкция'
    )
...
class WorkObject(models.Model):
    ...
    responsible_admin = models.ForeignKey(Admin, on_delete=models.SET_NULL, null=True, blank=True, related_name='managed_work_objects', verbose_name='Ответственный руководитель')
    created_by_admin = models.ForeignKey(
        Admin,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name='created_work_objects',
    ...
    def update_status(self):
        if self.status in ['cancelled', 'completed']:
            return self.status
        now = timezone.now()
        if self.status == 'waiting' and now >= self.time:
...
class MedicalCheck(models.Model):
    ...
    employee = models.ForeignKey(User, on_delete=models.CASCADE, related_name='medical_checks', verbose_name='Сотрудник')
    medic = models.ForeignKey(Medic, on_delete=models.SET_NULL, null=True, blank=True, related_name='medical_checks', verbose_name='Медработник')
...
class SickLeave(models.Model):
    ...
    issued_by = models.ForeignKey(Medic, on_delete=models.SET_NULL, null=True, blank=True, related_name='issued_sick_leaves', verbose_name='Выдал медработник')
...
class Question(models.Model):
    ...
    option3 = models.CharField('Вариант 3', max_length=200)
    correct_answer = models.IntegerField('Правильный ответ', choices=[(1, 'Вариант 1'), (2, 'Вариант 2'), (3, 'Вариант 3')])
...
class UserTestAttempt(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.passed and self.completed_at:
            self.user.certifications.add(self.test)
            self.user.assigned_certifications.remove(self.test)
...
class DocumentSignature(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='document_signatures', verbose_name="Пользователь")
...
class VideoInspection(models.Model):
    ...
    check_mask = models.BooleanField(default=True, verbose_name='Проверять маски')
